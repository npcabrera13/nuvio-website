...
export function AnimeRow({ anime, onOpenMovie }: AnimeRowProps) {
  const scroller = useRef<HTMLDivElement>(null);
  const list = anime.slice(0, 18);
    ...
    <section className="relative py-10 lg:py-14">
      <div className="mx-auto max-w-7xl px-4 sm:px-6">
        <div className="flex items-end justify-between gap-4 mb-6">
          <div>
            <p className="text-sm font-semibold uppercase tracking-wider text-fuchsia-400 flex items-center gap-1.5">
              <Sparkles className="h-4 w-4" /> Anime included
            </p>
          ...
          {list.map((m) => (
            ...
            >
              ...
              >
                ...
                />
                {/* Anime badge */}
                <span className="absolute top-2 left-2 inline-flex items-center gap-1 rounded-md bg-fuchsia-600/85 px-1.5 py-0.5 text-[10px] font-bold text-white backdrop-blur-sm">
                  <Sparkles className="h-2.5 w-2.5" /> ANIME
                </span>
                {m.imdbRating && (
